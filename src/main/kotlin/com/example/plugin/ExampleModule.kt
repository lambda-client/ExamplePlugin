        ...
        saveEntity.consumers.add {_, it ->
            if (it) {
                mc.objectMouseOver?.entityHit?.let {
                    entityToMount = it
                    MessageSendHelper.sendChatMessage("$chatName Entity saved ${it.positionVector} ID: ${it.entityId}")
                } ?: run {
        ...
        world.loadedEntityList
            .filter {
                ...
                if (maxReach == 0.0f || player.getDistance(it) <= maxReach) {
                    MessageSendHelper.sendChatMessage("$chatName Mounting: ${it.name}@${it.positionVector}")
                    connection.sendPacket(CPacketUseEntity(it, EnumHand.MAIN_HAND))
                } else {
                    MessageSendHelper.sendChatMessage("$chatName Closest entity too far away: ${it.name}@${it.positionVector.distanceTo(player.positionVector)}")
                }
            } ?: run {
            MessageSendHelper.sendChatMessage("$chatName Can't find any Entity in world.")
            }
    }
}